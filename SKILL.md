...
3. 如果检查提示首次启动、缺少 `config.yaml`、缺少 `sources.yaml`、`setup.initialized is not true`，或“尚未完成初始化向导”，告诉用户：`检测到 ai-news-keji 还没有完成初始化，我先带你完成初始化向导。` 然后在 Agent 对话里收集下面 5 项信息，不要开始抓取新闻。
4. 初始化向导问题：
   - 外部集成：是否安装 `follow-builders`、`BestBlogs`、`ak-rss-digest`；说明建议安装全部，也允许用户选择部分或都不安装。
   - Newsletter：展示 `sources.yaml` 或 `sources.example.yaml` 里的 Newsletter 名称、订阅地址和发件人白名单，然后让用户选择 `imap`、`later` 或 `no`。
   - IMAP：如果用户选择 `imap`，继续确认 IMAP host、folder、账号环境变量名、密码/授权码环境变量名，并提醒用户先订阅 Newsletter。
   - 输出目录：确认 Markdown 日报默认写入目录。
   - 个人偏好：引导用户输入关注主题、角色、项目、偏好的内容形式，以及要避开的内容；建议填写，空白时使用默认筛选逻辑。
5. 收集完答案后，把答案写入临时 JSON 文件，然后运行 `python3 scripts/init.py --answers-file <answers.json>`。JSON 字段：`external_skills`、`newsletter.choice`、`newsletter.host`、`newsletter.folder`、`newsletter.username_env`、`newsletter.password_env`、`output_dir`、`preferences`。
6. 写入配置后再次运行 `python3 scripts/init.py --check`。只有检查通过后，才继续读取 `config.yaml`、`sources.yaml` 并抓取新闻。
7. 不要安装可选外部 skills 或 CLI，除非用户在初始化回答里明确选择安装，或主动要求安装。
8. 解析路径时展开 `~` 和环境变量。
