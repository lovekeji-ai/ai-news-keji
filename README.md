...
- `scripts/check-run-state.py`：抓取前检查目标日期是否已有原始稿、摘要稿或缓存
...
开始抓取前，Agent 还会检查目标日期是否已经有运行产物：

```bash
.venv/bin/python scripts/check-run-state.py --date 2026-04-25 --config config.yaml
```

如果当天已经有原始稿、摘要稿或缓存，Agent 会先让用户选择：直接使用已有结果、补充抓取，或重新抓取并覆盖。
...
| 类型 | 示例 | 说明 |
| --- | --- | --- |
| RSS | 量子位、三花 AI 快讯 | 中文 AI 和科技媒体，适合作为每日基础盘 |
...
```bash
.venv/bin/python -m py_compile scripts/init.py scripts/init_wizard.py scripts/check-run-state.py scripts/fetch-rss.py scripts/fetch-email-imap.py scripts/doctor.py
.venv/bin/python scripts/doctor.py
```
